...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_input_: "bf16[1, 4608]"):
            ...
            x_q: "f8e4m3fn[1, 4608]" = torch.empty_like(input_2d, device = device(type='cuda', index=3), dtype = torch.float8_e4m3fn)
            ...
            x_s: "f32[1, 36]" = torch.empty((1, 36), device = device(type='cuda', index=3), dtype = torch.float32)
