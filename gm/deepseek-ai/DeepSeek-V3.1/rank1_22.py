...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_input_: "bf16[1, 7168]"):
            ...
            x_q: "f8e4m3fn[1, 7168]" = torch.empty_like(input_2d, device = device(type='cuda', index=1), dtype = torch.float8_e4m3fn)
            ...
            x_s: "f32[1, 56]" = torch.empty((1, 56), device = device(type='cuda', index=1), dtype = torch.float32)
    ...
    class inductor_1(torch.nn.Module):
        def forward(self, input_2d: "bf16[1, 7168]", x_q: "f8e4m3fn[1, 7168]", x_s: "f32[1, 56]"):
            ...
            triton_kernel_wrapper_mutation = torch.ops.higher_order.triton_kernel_wrapper_mutation(kernel_idx = 27, constant_args_idx = 38, grid = [(56, 1, 1)], tma_descriptor_metadata = {}, kwargs = {'y_ptr': input_2d, 'y_q_ptr': x_q, 'y_s_ptr': x_s});  input_2d = x_q = x_s = triton_kernel_wrapper_mutation = None
            return ()
