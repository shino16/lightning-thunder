...
class GraphModule(torch.nn.Module):
    def forward(self, l_forward_batch_next_token_logits_buffer: "f32[1, 129280]", l_self_modules_lm_head_parameters_weight_: "bf16[32320, 7168]", l_stack0_: "bf16[1, 7168]"):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_stack0_: "bf16[1, 7168]", l_self_modules_lm_head_parameters_weight_: "bf16[32320, 7168]"):
            ...
            to: "bf16[1, 7168]" = l_stack0_.to(torch.bfloat16);  l_stack0_ = None
            getattr_1: "bf16[7168, 32320]" = l_self_modules_lm_head_parameters_weight_.T;  l_self_modules_lm_head_parameters_weight_ = None
            
             # File: /opt/sglang/sglang-src/python/sglang/srt/layers/logits_processor.py:472 in _get_logits, code: logits = torch.matmul(
            logits: "bf16[1, 32320]" = torch.matmul(to, getattr_1);  to = getattr_1 = None
            ...
            output_tensor: "bf16[4, 32320]" = torch.empty((4, 32320), dtype = torch.bfloat16, device = device(type='cuda', index=0))
    ...
    class inductor_1(torch.nn.Module):
        def forward(self, logits: "bf16[1, 32320]", output_tensor: "bf16[4, 32320]"):
    ...
    class thunder_2(torch.nn.Module):
        def forward(self, output_tensor: "bf16[4, 32320]", l_forward_batch_next_token_logits_buffer: "f32[1, 129280]"):
            ...
            output_tensor_1: "bf16[4, 1, 32320]" = output_tensor.reshape((4, 1, 32320));  output_tensor = None
            ...
            output_tensor_2: "bf16[1, 4, 32320]" = output_tensor_1.movedim(0, 1);  output_tensor_1 = None
            ...
            output_tensor_3: "bf16[1, 129280]" = output_tensor_2.reshape((1, 129280));  output_tensor_2 = None
            ...
            getitem: "bf16[1, 129280]" = output_tensor_3[(slice(None, None, None), slice(None, 129280, None))];  output_tensor_3 = None
            copy_: "f32[1, 129280]" = l_forward_batch_next_token_logits_buffer.copy_(getitem);  l_forward_batch_next_token_logits_buffer = getitem = copy_ = None
            return ()
