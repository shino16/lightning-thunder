...
class GraphModule(torch.nn.Module):
    def forward(self, l_input_: "bf16[1, 4096]"):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_input_: "bf16[1, 4096]"):
            ...
            input_2d: "bf16[1, 4096]" = l_input_.view(-1, 4096);  l_input_ = None
            ...
            x_q: "f8e4m3fn[1, 4096]" = torch.empty_like(input_2d, device = device(type='cuda', index=3), dtype = torch.float8_e4m3fn)
            ...
            x_s: "f32[1, 32]" = torch.empty((1, 32), device = device(type='cuda', index=3), dtype = torch.float32)
    ...
    class inductor_1(torch.nn.Module):
        def forward(self, input_2d: "bf16[1, 4096]", x_q: "f8e4m3fn[1, 4096]", x_s: "f32[1, 32]"):
            ...
            triton_kernel_wrapper_mutation = torch.ops.higher_order.triton_kernel_wrapper_mutation(kernel_idx = 20, constant_args_idx = 29, grid = [(32, 1, 1)], tma_descriptor_metadata = {}, kwargs = {'y_ptr': input_2d, 'y_q_ptr': x_q, 'y_s_ptr': x_s});  input_2d = x_q = x_s = triton_kernel_wrapper_mutation = None
            return ()
