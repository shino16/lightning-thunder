...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_x_: "bf16[1, 5120]"):
            ...
            x_q: "f8e4m3fn[1, 5120]" = torch.empty_like(l_x_, device = device(type='cuda', index=0), dtype = torch.float8_e4m3fn);  l_x_ = None
            ...
            x_s: "f32[1, 1]" = torch.empty(1, 1, device = device(type='cuda', index=0), dtype = torch.float32)
            return (x_q, x_s)
