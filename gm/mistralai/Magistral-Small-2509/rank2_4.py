...
class GraphModule(torch.nn.Module):
    def forward(self, l_q_: "f16[1, 8, 128]", l_paged_kv_cache_0_: "f16[3432731, 2, 128]", l_paged_kv_cache_1_: "f16[3432731, 2, 128]"):
    ...
    class thunder_1(torch.nn.Module):
        def forward(self, l_paged_kv_cache_0_: "f16[3432731, 2, 128]", l_paged_kv_cache_1_: "f16[3432731, 2, 128]", l_q_: "f16[1, 8, 128]"):
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/utils.py:89 in _expand_4d, code: return x.unsqueeze(-3)
            k_cache: "f16[3432731, 1, 2, 128]" = l_paged_kv_cache_0_.unsqueeze(-3);  l_paged_kv_cache_0_ = None
            
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/utils.py:89 in _expand_4d, code: return x.unsqueeze(-3)
            v_cache: "f16[3432731, 1, 2, 128]" = l_paged_kv_cache_1_.unsqueeze(-3);  l_paged_kv_cache_1_ = None
            
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/decode.py:1273 in run, code: out = torch.empty_like(q)
            out: "f16[1, 8, 128]" = torch.empty_like(l_q_);  l_q_ = None
            ...
            buf: "f32[8]" = float_1.to(device(type='cuda', index=2));  float_1 = None
