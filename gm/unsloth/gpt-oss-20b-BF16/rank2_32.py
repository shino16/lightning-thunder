...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self):
            ...
            output: "bf16[1, 4, 720]" = torch.empty((1, 4, 720), device = device(type='cuda', index=2), dtype = torch.bfloat16)
            ...
            getitem: "bf16[1, 1, 4, 720]" = output[(None, slice(None, None, None), slice(None, None, None))];  output = None
            ...
            empty_1: "f32[11, 1, 4, 1440]" = torch.empty((11, 1, 4, 1440), device = device(type='cuda', index=2), dtype = torch.float32)
