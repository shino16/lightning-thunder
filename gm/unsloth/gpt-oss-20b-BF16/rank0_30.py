...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_stack0_2_scratchpad: "i32[128]"):
            ...
            out_partials: "i32[32, 4]" = torch.empty((32, 4), device = device(type='cuda', index=0), dtype = torch.int32)
    ...
    class inductor_1(torch.nn.Module):
        def forward(self, l_stack0_2_storage_data: "u32[1, 1]", out_ret: "i32[32]", out_partials_1: "i32[4, 32]"):
            ...
            triton_kernel_wrapper_mutation = torch.ops.higher_order.triton_kernel_wrapper_mutation(kernel_idx = 10, constant_args_idx = 8, grid = [(1, 1, 1)], tma_descriptor_metadata = {}, kwargs = {'B': l_stack0_2_storage_data, 'Ret': out_ret, 'Partials': out_partials_1});  l_stack0_2_storage_data = out_ret = out_partials_1 = triton_kernel_wrapper_mutation = None
            return ()
    ...
    class thunder_2(torch.nn.Module):
        def forward(self, out_partials_1: "i32[4, 32]", out_ret: "i32[32]"):
            ...
            expt_offs: "i32[32]" = torch.empty(32, dtype = torch.int32, device = device(type='cuda', index=0))
            ...
            combined_indx: "i32[8]" = torch.empty(8, dtype = torch.int32, device = device(type='cuda', index=0))
            ...
            gate_scal: "bf16[4]" = torch.empty(4, dtype = torch.bfloat16, device = device(type='cuda', index=0))
            ...
            token_offs_combined: "i32[5, 512]" = torch.empty((5, 512), dtype = torch.int32, device = device(type='cuda', index=0))
            ...
            block_pid_map: "i32[4, 512]" = torch.empty((4, 512), dtype = torch.int32, device = device(type='cuda', index=0))
