...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self):
            ...
            y_vals: "bf16[1, 4]" = torch.empty((1, 4), dtype = torch.bfloat16, device = device(type='cuda', index=1))
            ...
            y_indx: "i16[1, 4]" = torch.empty((1, 4), dtype = torch.int16, device = device(type='cuda', index=1))
    ...
    class inductor_1(torch.nn.Module):
        def forward(self):
            ...
            bitmatrix: "u32[1, 32]" = torch.empty((1, 32), dtype = torch.uint32, device = device(type='cuda', index=1))
    ...
    class thunder_2(torch.nn.Module):
        def forward(self):
            ...
            scratchpad: "i32[128]" = torch.empty((128,), dtype = torch.int32, device = device(type='cuda', index=1))
            return scratchpad
