...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self, l_stack0_2_scratchpad: "i32[128]"):
            ...
            out_partials: "i32[32, 4]" = torch.empty((32, 4), device = device(type='cuda', index=1), dtype = torch.int32)
    ...
    class thunder_2(torch.nn.Module):
        def forward(self, out_partials_1: "i32[4, 32]", out_ret: "i32[32]"):
            ...
            expt_offs: "i32[32]" = torch.empty(32, dtype = torch.int32, device = device(type='cuda', index=1))
            ...
            combined_indx: "i32[8]" = torch.empty(8, dtype = torch.int32, device = device(type='cuda', index=1))
            ...
            gate_scal: "bf16[4]" = torch.empty(4, dtype = torch.bfloat16, device = device(type='cuda', index=1))
            ...
            token_offs_combined: "i32[5, 512]" = torch.empty((5, 512), dtype = torch.int32, device = device(type='cuda', index=1))
            ...
            block_pid_map: "i32[4, 512]" = torch.empty((4, 512), dtype = torch.int32, device = device(type='cuda', index=1))
