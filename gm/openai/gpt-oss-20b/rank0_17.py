...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self):
            ...
            out_val: "f8e4m3fn[1, 3072]" = torch.empty((1, 3072), dtype = torch.float8_e4m3fn, device = device(type='cuda', index=0))
            ...
            out_sf: "u8[96]" = torch.empty((96,), dtype = torch.uint8, device = device(type='cuda', index=0))
