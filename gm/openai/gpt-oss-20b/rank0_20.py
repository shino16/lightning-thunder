...
class GraphModule(torch.nn.Module):
    ...
    class thunder_0(torch.nn.Module):
        def forward(self):
            ...
            topk_ids: "i32[1, 4]" = torch.empty(1, 4, dtype = torch.int32, device = device(type='cuda', index=0))
            ...
            expert_weights: "bf16[1, 4]" = torch.empty(1, 4, dtype = torch.bfloat16, device = device(type='cuda', index=0))
    ...
    class thunder_2(torch.nn.Module):
        def forward(self):
            ...
            output: "bf16[1, 3072]" = torch.empty(1, 3072, dtype = torch.bfloat16, device = device(type='cuda', index=0))
            return output
