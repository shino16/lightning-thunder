...
class GraphModule(torch.nn.Module):
    def forward(self, l_q_: "bf16[1, 10, 128]", l_paged_kv_cache_0_: "bf16[3716338, 2, 128]", l_paged_kv_cache_1_: "bf16[3716338, 2, 128]"):
    ...
    class thunder_1(torch.nn.Module):
        def forward(self, l_paged_kv_cache_0_: "bf16[3716338, 2, 128]", l_paged_kv_cache_1_: "bf16[3716338, 2, 128]", l_q_: "bf16[1, 10, 128]"):
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/utils.py:89 in _expand_4d, code: return x.unsqueeze(-3)
            k_cache: "bf16[3716338, 1, 2, 128]" = l_paged_kv_cache_0_.unsqueeze(-3);  l_paged_kv_cache_0_ = None
            
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/utils.py:89 in _expand_4d, code: return x.unsqueeze(-3)
            v_cache: "bf16[3716338, 1, 2, 128]" = l_paged_kv_cache_1_.unsqueeze(-3);  l_paged_kv_cache_1_ = None
            
             # File: /usr/local/lib/python3.12/dist-packages/flashinfer/decode.py:1273 in run, code: out = torch.empty_like(q)
            out: "bf16[1, 10, 128]" = torch.empty_like(l_q_);  l_q_ = None
            ...
            buf: "f32[10]" = float_1.to(device(type='cuda', index=2));  float_1 = None
